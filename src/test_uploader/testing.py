...
class Downloader(webapp.RequestHandler):
    def get(self):
        ...
        t=datetime.strptime((self.request.GET.get('time')), '%Y-%m-%d %H:%M:%S')
        rd=int(self.request.GET.get('delay'))
        rc=int(self.request.GET.get('crowd'))
        rh=int(self.request.GET.get('happy'))
